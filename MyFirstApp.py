...
if app_mode=='Home':
    ...
    st.image('source/loan_img.png')
    st.markdown('Dataset :')
    data = pd.read_csv('source/loan_dataset.csv')
    st.write(data.head())
    st.markdown('Applicant Income VS Loan Amount ')
    st.bar_chart(data[['ApplicantIncome','LoanAmount']].head(20))
elif app_mode == 'Prediction':
    st.image('source/kid.jpg')
    ...
    if st.button("Predict"):
        file_ = open("source/6m-rain.gif", "rb")
        contents = file_.read()
        data_url = base64.b64encode(contents).decode("utf-8")
        file_.close()

        file = open("source/green-cola-no.gif", "rb")
        contents = file.read()
        data_url_no = base64.b64encode(contents).decode("utf-8")
        file.close()

        loaded_model = pickle.load(open("source/Random_Forest_p.sav", "rb"))
        #loaded_model = joblib.load("source/Random_Forest.pkl")
        ...
        if prediction[0] == 0 :
            ...
            st.markdown(
                f'<img src="data:image/gif;base64,{data_url_no}" alt="cat gif">',unsafe_allow_html=True,
                )
        elif prediction[0] == 1 :
            st.success(
